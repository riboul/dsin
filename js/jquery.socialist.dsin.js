        ...
        var methods = {
            ...
            init : function(options) {
                ...
                return this.each(function() {
                    ...
                    if (settings.feed) {
                        processList.push(helpers.doRequest(settings.feed,"jsonp",function(q){
                    ...
                    $.when.apply($, processList).always(function(){
						if (queue.length > 0){
							// quand le proxy bloque il faut verifier le contenu
							// pour ne pas afficher le bloc car les feeds ne remontent pas
							$element.show();
							/*
							if (settings.screenSize < 601){
								$('#content').removeClass("threeQuarterWidth").addClass("threeQuarterWidth");
							} else {
								$('#content').removeClass("threeQuarterWidth");
							}
							*/
						}
        ...
        var helpers = {
                ...
                $.each(eval(apiParser.resultsSelector), function(i,item) {
                    ...
                    try{
                        ...
                        if (eval(apiParser.preCondition)) {
							...
							var nombre = container.children().length;
							nombre = nombre%2 + 1;
							
							$iconBackground = $('<img class="social-background" src="images/social/back'+ nombre +'_flux_clear.png" alt="row-icon" />');
							$iconBackground.appendTo($div);
                            
                            $div = helpers.buildItem(itemObj,$div,settings.fields);

							$iconRow = $('<img class="social-rowIcon" src="images/social/'+ apiParser.name + nombre +'_clear.png" alt="row-icon" />');
							$iconRow.appendTo($div);
                            $div.appendTo(container);
                        }
            ...
            buildItem: function(itemObj,container,fields) {
                ...
                var $headDiv = $('<div class="head"/>'),
                    ...
                    $contentDiv = $('<div class="content">'+itemObj.txt+'</div>'),
            ...
            networkDefs: {
                ...
                facebook_search:{url:'https://graph.facebook.com/|id|/feed?access_token=325618697471211|oka_aeIaMJ19F-8rM5sS2jB9AJs&limit=|num|',img:'',dataType:'jsonp',parser:{
                    name: "facebook",
                    resultsSelector: "data.data",
                    heading: "Facebook",
                    headingSelector: "item.from.name",
                    txtSelector: "(item.message)||item.description",
                    dateSelector: "helpers.fixFacebookDate(item.created_time)",
                    imgSrcSelector: "(item.picture)||'/spacer.gif'",
                    imgSrcProcessor: null,
                    imgHrefSelector: "item.link",
                    imgAltSelector: "item.from.name.substring(0,12)",
                    link: "#",
                ...
                twitter:{url:'https://api.twitter.com/1/statuses/user_timeline.json?include_entities=true&include_rts=true&screen_name=|id|&count=|num|',dataType:"jsonp",img:'',parser:{
                    ...
                    dateSelector: "helpers.fixTwitterDate(item.created_at)",
                ...
                linkedin:{url:'https://www.linkedin.com/company/|id|',dataType:"text",parser:{
                    name: "linkedin",
			...
			fixFacebookDate: function(created_at){
				created_at = created_at.substr(0,10);
				day = created_at.substr(8,2);
				month = created_at.substr(5,2);
				year = created_at.substr(0,4);
